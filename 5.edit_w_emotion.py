...
def main():
    ...
    instruction_dir = "data/4.instruction"
    output_dir = "data/5.emotionE"

    instrcution_file_count = 5
    ...
    for file_name in tqdm(list(os.listdir(input_dir))):
        ...
        for instrcution_file_idx in tqdm(list(range(instrcution_file_count))):
            instruction_path = os.path.join(instruction_dir, f"{file_name}.{instrcution_file_idx}.txt")

            while not os.path.exists(instruction_path):
                time.sleep(10)
            ...
            for emotion, instructions in instruction_dict.items():
                ...
                assert len(instructions) == 3 and all(isinstance(instruction, str) for instruction in instructions)
